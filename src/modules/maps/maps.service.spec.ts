...
describe("MapsService", () => {
  ...
  describe("MapsService", () => {
    ...
    describe("calculateDriveTime", () => {
      it("should calculate drive time between two addresses", async () => {
        mockHttpClient.post.mockResolvedValueOnce({
          data: {
            routes: [
              {
        ...
        const result = await service.calculateDriveTime("Ikeja, Lagos", "Victoria Island, Lagos");
      ...
      it("should return fallback on API error", async () => {
        const axiosError = createAxiosErrorWithResponse(HttpStatus.BAD_REQUEST, {
          message: "Bad request",
        });
        mockHttpClient.post.mockRejectedValueOnce(axiosError);

        const result = await service.calculateDriveTime("Invalid", "Also Invalid");
      ...
      it("should return fallback when no routes found", async () => {
        ...
        const result = await service.calculateDriveTime("Unknown Place", "Another Unknown");
      ...
      it("should return fallback on network error", async () => {
        const axiosError = createAxiosErrorWithRequest("Connection refused");
        mockHttpClient.post.mockRejectedValueOnce(axiosError);

        const result = await service.calculateDriveTime("Ikeja", "Victoria Island");
