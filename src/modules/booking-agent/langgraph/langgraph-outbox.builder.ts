...
export function buildOutboxItems(
  ...
  templates: {
    vehicleCardContentSid?: string;
    checkoutLinkContentSid?: string;
  } = {},
): LangGraphOutboxItem[] {
  ...
  if (response.vehicleCards && response.vehicleCards.length > 0) {
    ...
    if (templates.vehicleCardContentSid) {
      response.vehicleCards.forEach((card, index) => {
        ...
        outboxItems.push({
          ...
          templateName: templates.vehicleCardContentSid,
          templateVariables,
        });
      });
    }
  ...
  if (state.stage === "awaiting_payment" && state.paymentLink) {
    ...
    if (!checkoutToken || !templates.checkoutLinkContentSid) {
      return [
        {
          conversationId: state.conversationId,
    ...
    return [
      {
        ...
        mode: LANGGRAPH_OUTBOUND_MODE.TEMPLATE,
        templateName: templates.checkoutLinkContentSid,
        templateVariables: {
          "1": response.text,
          "2": checkoutToken,
        },
