...
export class StatusChangeEventsListener {
  ...
  private async handleBookingConfirmed(payload: BookingConfirmedEventPayload): Promise<void> {
  ...
  async onFlightArrivalUpdated(payload: FlightArrivalUpdatedEventPayload): Promise<void> {
    return observeBackgroundOperation(
      "StatusChangeEventsListener.onFlightArrivalUpdated",
      "event",
      () => this.handleFlightArrivalUpdated(payload),
    );
  }

  private async handleFlightArrivalUpdated(
    payload: FlightArrivalUpdatedEventPayload,
  ): Promise<void> {
    const activationAt = new Date(payload.activationAt);
    if (Number.isNaN(activationAt.getTime())) {
      this.logger.warn(
